...
__constant__ float C_kernel[125];

__global__ void Convolution(float* N, float* P, float* C, int x, int y, int z, int MASK_WIDTH){
        int tx = threadIdx.x;
        int ty = threadIdx.y;
        int tz = threadIdx.z;
	int TILE_SIZE = BLOCK_SIZE-MASK_WIDTH+1;
        ...
        if((row_i>=0)&&(row_i<Rows)&&(col_i>=0)&&(col_i<Columns)&&(dep_i>=0)&&(dep_i<Depth)){
                Ns[tz*Rows*Columns+ty*Columns+tx] = N[dep_i*Rows*Columns+row_i*Columns+col_i];//thisk;
        }
        else{
                Ns[tz*Rows*Columns+ty*Columns+tx] = 0.0f;
        }
        __syncthreads();
                ...
                for(int i=0;i<MASK_WIDTH;i++){
                        for(int j=0;j<MASK_WIDTH;j++){
                                for(int k=0;k<MASK_WIDTH;k++){
                                        output += C_kernel[i*MASK_WIDTH*MASK_WIDTH+j*MASK_WIDTH+k]*Ns[(i+tz)*Rows*Columns+(j+ty)*Columns+(k+tx)];
                                }
                        }
                }
...
int main(int argc, char *argv[]){
	...
	CPY = (float*)malloc(sizeof(float)*kernel_size*kernel_size*kernel_size);
	...
	cudaMemcpyToSymbol(C_kernel, KERNEL, sizeof(float)*125);

        cudaMalloc((void**)&IN_c, sizeof(float)*input_x*input_y*input_z);
        cudaMalloc((void**)&OUT_c, sizeof(float)*output_x*output_y*output_z);
	cudaMalloc((void**)&CPY_c, sizeof(float)*kernel_size*kernel_size*kernel_size);
        ...
        Convolution<<<dimGrid,dimBlock, (TILE_SIZE+kernel_size-1)*(TILE_SIZE+kernel_size-1)*(TILE_SIZE+kernel_size-1)>>>(IN_c,OUT_c, CPY_c,  input_x, input_y, input_z, kernel_size);
        cudaMemcpy(OUT, OUT_c, sizeof(float)*output_x*output_y*output_z, cudaMemcpyDeviceToHost);
	cudaMemcpy(CPY, CPY_c, sizeof(float)*kernel_size*kernel_size*kernel_size, cudaMemcpyDeviceToHost);
	for(int i=0;i<kernel_size*kernel_size*kernel_size;i++){
		printf(" %f ", CPY[i]);
	}
	printf("\n");
